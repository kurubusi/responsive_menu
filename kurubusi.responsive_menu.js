...
KURUBUSI.modules.func = function(K){
	...
	K.addRemoveEvent = function(elm,listener,fn){
...
KURUBUSI.modules.app = function(K){
	 ...
	 K.responsiveMenu = function(){
		...
		var ResponsiveMenu = function(button){
			...
			K.addEventSet(this.buttonelm, 'click', function(event){
				this_.fadeSpNav(event);
			});
			K.addEventSet(this.transparentdiv, 'click', function(event){
				this_.close(this_.fadeto);
			});
			K.addEventSet(this.transparentdiv, 'touchstart', function(event){
				this_.close(this_.fadeto);
			});
			try{
				K.addEventSet(this.transparentdiv, 'mousewheel', function(event){
					if (event.preventDefault) {
						event.preventDefault();
					}
					event.returnValue = false;
				...
				K.addEventSet(this.transparentdiv, 'mousewheelevent', function(event){
					if (event.preventDefault) {
						event.preventDefault();
					}
					event.returnValue = false;
		...
		ResponsiveMenu.prototype.close = function(event){
			...
			K.addRemoveEvent(document , 'scroll', this_.scrollwindow);
		...
		ResponsiveMenu.prototype.open = function(event){
			...
			K.addEventSet(document , 'scroll', this_.scrollwindow);
		...
		var buttonElem = K.switchArea('k_nav_b'),
				navElem = K.switchArea('k_target_obj'),
				breakpoint = 800,
				navarr = [];
